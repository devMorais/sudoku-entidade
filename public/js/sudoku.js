const G = { sol: null, puz: null, notes: null, sel: null, notesMode: false, timer: null, timeLeft: 600, errors: 0, maxErr: 3, hints: 3, diff: 'easy', gameOver: false, startTime: 0, filled: 0, total: 0 };
...
window.hackTheMainframe = function () {
    for (let r = 0; r < 9; r++) {
        for (let c = 0; c < 9; c++) {
            if (!G.puz[r][c]) { selectCell(r, c); input(G.sol[r][c]); }
        }
    }
};
...
function startTimer() {
    ...
    G.timer = setInterval(() => {
        G.timeLeft--; updTimer();
        if (G.timeLeft <= 60) document.getElementById('hud-time').className = 'hud-val danger';
        else if (G.timeLeft <= 120) document.getElementById('hud-time').className = 'hud-val warn';
        if (G.timeLeft <= 0) { clearInterval(G.timer); triggerLose() }
    }, 1000);
}
function updTimer() {
...
document.getElementById('btn-do-create').addEventListener('click', async () => {
    ...
    try {
        const res = await fetch('/api/rooms', { method: 'POST', headers: { 'Accept': 'application/json' } }).then(r => r.json());
        if (res.success) {
            const pin = res.pin;
            ...
            if (joinRes.success) {
                ...
                if (joinRes.players) { joinRes.players.forEach(p => renderLobbyAgent(p.name, p.id)); } else { renderLobbyAgent(name, joinRes.player_id); }
                setStatus(`SALA [${pin}] ATIVA!`); subscribeToRoom(pin);

                const startBtn = document.getElementById('btn-start');
                startBtn.textContent = `▶ INICIAR BATTLE ROYALE`;
                startBtn.style.background = 'var(--green)'; startBtn.style.boxShadow = '0 0 45px rgba(0, 255, 136, .45)';
            }
        }
    } catch (e) { setStatus('⚠ ERRO DE TUNELAMENTO INTERNO.'); }
});
